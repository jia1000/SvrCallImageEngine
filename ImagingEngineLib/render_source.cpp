	...
	case DW::IMAGE::RenderControlType::VR:
		{
			...
			////TODO 将RenderMode和BlendMode接口从param类中移植到Renderer类中
	...
	case DW::IMAGE::RenderControlType::CROSS_SECTIONAL_MPR:
		{
			...
			if (ConfigurationManager::Get()->GetMPRRenderMethod() == "VTK_CPU"){
				renderer = new MPRRendererVtk();

				CGLogger::Info("Create mpr renderer with VTK_CPU");
			}
			else{
				renderer = new MPRRenderer();

				CGLogger::Info("Create mpr renderer with CPU");
			}			
