/*=========================================================================
  ...
  Module:    ct_image_storage_sop_generator.cpp
  author: 	 zhangjian
  Brief:	 

=========================================================================*/
#include "ct_image_generator.h"
#include "../iod/ct_image_module.h"
#include "../iod/frame_of_reference_module.h"
...
#include "../iod/image_plane_module.h"
...
namespace GIL {
	namespace DICOM {
		...
		CTImageDcmGenerator::CTImageDcmGenerator(DicomDataset *dataset)
			: IDcmGenerator(dataset)
		{
			Initialize();
		}

		CTImageDcmGenerator::~CTImageDcmGenerator()
		{
		...
		void CTImageDcmGenerator::Initialize()
		{
			// Patient
			...
			meta_info_module_->SetMediaStorageSOPClassUID("1.2.840.10008.5.1.4.1.1.2");
			meta_info_module_->SetMediaStorageSOPInstanceUID("1.0.0.1");
		...
		int CTImageDcmGenerator::Write(std::string &path)
		{
			// SOP Class UID
			dicom_file_->getDataset()->putAndInsertString(ToDcmTag(DCM_SOPClassUID), "1.2.840.10008.5.1.4.1.1.2");
			// 删除不属于1.1.2图像的tag
			dicom_file_->getDataset()->remove(ToDcmTag(DCM_PlanarConfiguration));
			// 写入文件需要指定传输语法
		...
		OFCondition CTImageDcmGenerator::ReadAndInsertPixelData(DcmDataset* dset,
