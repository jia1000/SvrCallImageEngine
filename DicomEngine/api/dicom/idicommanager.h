...
namespace GIL {
	namespace DICOM {
		...
		public:
			...
			virtual int LoadPrivateTags(GIL::DICOM::TipoPrivateTags& tags)=0;
			
			/*carga la MetaInfo*/
			virtual int CargarMetaInfo(const std::string& inputFile, GIL::DICOM::TipoMetaInfo& tags)=0;
			//virtual int GenerateKeySeriesAndGSPS(std::vector<std::string>& inputFileList, DW::MODEL::AIResult* aiResult, const std::string& aiInfor, std::string destFolder, boolean isGenerateKeySeries, boolean isGenerateGSPS, std::list<string>& listOfFileToUpload, std::string& seriesUID)=0;
